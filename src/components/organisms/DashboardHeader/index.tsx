...
const DashboardHeader = (props: DashboardHeaderProps) => {
  ...
  return (
    <header className="flex flex-col-reverse justify-between gap-4 md:flex-row">
      <div className="space-y-2 text-sm text-suite-dark xl:text-xl">
        <h4 className="text-4xl font-bold text-black">Hello, {firstName}</h4>
        <p>{headerDesc}</p>
      </div>
      <div className="flex items-start gap-4">
        <button className="flex h-[50px] w-[50px] items-center justify-center rounded-full bg-gray">
          <svg
            ...
            fill="none"
          >
            <g clip-path="url(#clip0_1693_4954)">
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
        ...
        <Avatar email={email} name={cn(firstName, lastName)} />
      </div>
    </header>
  );
};

    ...
    <div className="space-y-3">
      {rentHistory.length > 0 ? (
        ...
        <EmptyScreen
          ...
          desc="You are yet to receive any rent payment"
        />
      )}
    </div>
  );
...
export const ManualRentCollectionTab = () => {
  ...
  const isNotPending = suites.filter((suite) => {
    const notPending = suite.tenant.manual_payments.some(
  ...
  return (
    <div className="space-y-3">
      {isNotPending ? (
        ...
        <EmptyScreen
          title="No manual rent record"
          desc="Manual payments would appear here when you approve or reject them"
          icon={
            <IconBox
